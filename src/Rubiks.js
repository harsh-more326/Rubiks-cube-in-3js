...
function newCube(x, y, z) {
  const geometry = new THREE.BoxGeometry(CUBE_SIZE, CUBE_SIZE, CUBE_SIZE,12,12,12);
...
function rotateRow(fixedCoordinate, axis, clockwise) {
  ...
  cubesToRotate.forEach(cube => {
    ...
    switch (axis) {
      case 'x':
        targetRotation.x = clockwise ? cube.rotation.x + Math.PI / 2 : cube.rotation.x - Math.PI / 2;
        break;
      case 'y':
        targetRotation.y = clockwise ? cube.rotation.y + Math.PI / 2 : cube.rotation.y - Math.PI / 2;
        break;
      case 'z':
        targetRotation.z = clockwise ? cube.rotation.z + Math.PI / 2 : cube.rotation.z - Math.PI / 2;
        break;
    }
    rotationTween.to(targetRotation, 250).onComplete(() => {
